...
function [u1, u2, post_ord,ch, Px, Py,I, nflops, neighbor] = Stirling_scaling(r, x, y, q, scaling)
...
for i = post_ord
    ...
    if lvl >= 2
        if isempty(ch{i})
            xi = x(Px{i});
            Vi = particle_to_local(xi, r, I(i,1), 2*I(i,2), scaling);
            v1{i} = Vi' * q(Px{i}, :);
            nflops = nflops + size(Vi,2)*(2*size(Vi,1)-1)*size(q,2);
        else
            c1 = ch{i}(1); 
            c2 = ch{i}(2);
            Wc1 = multipole_to_multipole( r, I(c1,1), I(i,1), 2*I(c1,2), 2*I(i,2), scaling);  % Stirling scaling
            Wc2 = multipole_to_multipole( r, I(c2,1), I(i,1), 2*I(c2,2), 2*I(i,2), scaling);  % upward pass
...
for i = pre_ord
    ...
    if lvl >= 2
        list = interlist{i};
        for j = list
        [Bij, B2] = multipole_to_local(r, I(i,1), I(j,1),2*I(i,2),2*I(j,2), scaling); % Stirling scaling 
             if isempty(u1{i})
                u1{i} = Bij * v1{j};
                u2{i} = B2 * v1{j};
        ...
        if lvl > 3
            p = tr(i);
            if ~isempty(u1{p})
                Ri = multipole_to_multipole(r, I(i,1), I(p,1),  2*I(i,2), 2*I(p,2), scaling); % downward pass
                if isempty(u1{i})
                    u1{i} = Ri * u1{p};
                    u2{i} = Ri * u2{p};
